...
function renderView() {
  ...
  // Bind card clicks → profile drawer on leaderboard
  container.querySelectorAll('article.mc').forEach(card => {
    const memberId = card.dataset?.id || card.closest('[data-id]')?.dataset?.id;
    if (!memberId) return;
    card.style.cursor = 'pointer';
    card.addEventListener('click', () => window.location.href = `leaderboard.html`);
  });
}
